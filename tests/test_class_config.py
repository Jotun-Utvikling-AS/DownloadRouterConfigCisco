...
def test_config():
    ...
    config = Config()
    assert config.app_dict["author"] == "Jørn Larsen <jorn.larsen@jotun-utvikling.no>"
    assert re_match("\\d{4}(-\\d{2}){2}", "2024-12-27")
    ...
    assert config.app_dict["url"] == "https://github.com/Jotun-Utvikling-AS/DownloadRouterConfigCisco"
